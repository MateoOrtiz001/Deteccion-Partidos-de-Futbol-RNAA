...
class EstimadorMovimientoCam():
    ...
    def get_camera_movement(self,frames,readFromStub=False, stubPath=None):
        ...
        for frameNum in range[1,len(frames)]:
            ...
            newFeatures, _,_ = cv2.calcOptilcalFlowPyrLK(oldGray, frameGray, None, **self.lk_params)
            
            maxDistance = 0
            cameraMov_x ,cameraMov_y = 0,0
            
            for i, (new,old) in enumerate(zip(oldFeatures,oldFeatures)):
                newFeatures_point = new.ravel()
                oldFeatures_point = new.ravel()
    ...
    def draw_camera_movement(self, frames, camera_movement_per_frame):
        ...
        for frameNum, frame in enumerate(frames):
            ...
            frame = cv2.putText(frame,f"Movimiento de cámara X: {xMov: 2f}", (10,30), cv2.FONT_HERSHEY_SIMPLEX,1 (0,0,0),3)
            frame = cv2.putText(frame,f"Movimiento de cámara Y: {yMov: 2f}", (10,60), cv2.FONT_HERSHEY_SIMPLEX,1 (0,0,0),3)
